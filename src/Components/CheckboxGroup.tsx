...
export const CheckboxGroup: React.FC<any> = ({
  ...
  const validationMinSelections =
    required && minSelections === undefined ? 1 : minSelections || 0;
  const validationMaxSelections =
    maxSelections === undefined ? options.length : maxSelections;
  ...
  // --- useEffect to re-sync internal state with the prop ---
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    selectedValues,
    validationMinSelections,
    validationMaxSelections,
    required,
  ]);
  ...
  return (
    <>
      ...
      <button type="button" onClick={handlReset}>
        Reset to Defaults
    </button>
    </>
  );
